...
const UpdateProduct = () => {
    ...
    const ProductDetails =async()=>{
        let result = await fetch(`http://localhost:5000/product/single-product/${params.id}`,{
            headers:{
    ...
    const update = async()=>{
       let result = await fetch(`http://localhost:5000/product/update-product/${params.id}`,{
        method:'PUT',
        body:JSON.stringify({
             productName,
                productType,
